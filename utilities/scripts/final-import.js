...
/**
 * Final Import Script
 * Clears all tables and imports data
 */
...
console.log('Clearing all tables and importing data...\n');
...
function removeDateFields(obj) {
  ...
  for (const [key, value] of Object.entries(cleaned)) {
    ...
    if (key === 'created_at' || key === 'updated_at' || key === 'createdAt' || key === 'updatedAt' ||
        ...
        key === 'bannedAt' || key === 'banned_at' || key === 'completedAt' || key === 'completed_at' ||
        key === 'last_seen' || key === 'last_activity' || key === 'joined_at' ||
        key === 'last_message' || key === 'last_interaction' || key === 'lastAttempt' || key === 'last_attempt' ||
